...
tsNow = dtNow.strftime('%Y%m%d%H%M%S') # epoch
epochNow = dtNow.timestamp() # epoch
...
def xtrInfos(candidat, line, dP) :
    ...
    if (candidat == 'IIVTG') :
        ...
        if (lTmp[9][0] == "A") :
            ...
            dP['IIVTGtmg'].append(round(float(lTmp[1])))
            dP['IIVTGsog'].append(float(lTmp[5]))    
        # return candidat + " = pas necessaire"
        return candidat + " dans " + line
    if (candidat == 'IIDPT') :
        # $IIDPT,0001.9,,*7A
